...
const Card = ({ children }: { children: React.ReactNode }) => (
  <div className="border rounded p-4 bg-white shadow-sm mb-4">{children}</div>
);
const CardContent = ({ children }: { children: React.ReactNode }) => <div>{children}</div>;
const Button = ({ children, onClick }: { children: React.ReactNode; onClick: () => void }) => (
  <button className="bg-blue-500 text-white px-4 py-2 rounded mt-2" onClick={onClick}>{children}</button>
);
    ...
    <div className="p-6 space-y-8 font-sans max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {data.metadataBadges.map((badge, index) => (
          <span key={index} className="bg-gray-200 px-3 py-1 rounded-full">{badge}</span>
        ))}
      </div>
